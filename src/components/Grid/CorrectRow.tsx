...
export const CorrectRow: React.FC<RowProps> = ({ onUpdate }) => {
  ...
  const classes: string =
    "font-bold xxshort:w-9 xxshort:h-9 short:text-xl short:w-10 short:h-10 w-12 h-12 border-solid border-2 flex items-center justify-center mx-0.5 text-xl font-bold rounded dark:text-white text-center uppercase rounded box-border border-green-500 focus:border-green-700 focus:caret-green focus:ring-green";
  ...
  return (
    <>
      <h3 className="mt-4 text-[16px] font-semibold">🟩 Correct letters</h3>
      <p className="text-sm">
      ...
      <div className="relative min-w-max input mt-2">
        ...
        {!isEmpty && (
          <div className="text-right">
            {isError && (
              <div className="text-right text-red-500">{isError}</div>
            )}
            <button
              type="reset"
              className="clear-all-link !pr-0"
              onClick={handleClear}
            >
              Clear all
            </button>
